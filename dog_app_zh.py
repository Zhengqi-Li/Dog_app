...
for path in human_files_short:
    human_results.append(face_detector(path))
for path in dog_files_short:
    dog_results.append(face_detector(path))
print('human_files 的前100张图像中，能够检测到人脸的比例:{:.2f}%'.format(Counter(human_results)[True]/len(human_results)*100))
print('dog_files 的前100张图像中，能够检测到人脸的比例:{:.2f}%'.format(Counter(dog_results)[True]/len(dog_results)*100))
...
# 
# __回答:__
# * 不是很合理，因为用户提供的人脸图像清晰度可能由于获取图像的手段不同而产生变化
# * 可以调节 face_cascade.detectMultiScale() 中的 minNeighbors 参数，当 minNeighbors 越小时，算法检测人脸的条件更为宽松，即使是不清晰的图像，也会提高检测到人脸的几率，但是准确率可能会受到影响
...
### 你可以随意使用所需的代码单元数
def face_detector_refine(img_path):
    img = cv2.imread(img_path)
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
...
r1 = []
r2 = []
human_faces = human_files[:1000]
for path in human_faces:
    r1.append(face_detector(path))
    r2.append(face_detector_refine(path))
print('r1_results:{:.2f}%'.format(Counter(r1)[True]/len(r1)*100))
print('r2_results:{:.2f}%'.format(Counter(r2)[True]/len(r2)*100))
...
r3 = []
for path in dog_files_short:
    r3.append(face_detector_refine(path))
print('r3_results:{:.2f}%'.format(Counter(r3)[True]/len(r3)*100))


# * 说明：在上述程序中，将 detectMultiScale() 中的 minNeighbors 参数由3（默认值）改为1，输入是人脸图像中的前1000张，人脸识别准确率从98.40%提升至99.20%，但是将狗的照片作为输入，识别为人脸的错误率也提升了（由11%上升至34%）
# * 但从用户角度上看，给模型的一定是人脸的图像，排除了误将其他照片识别为人脸的可能性，尽管是不清晰的图像，也能进行较好的识别
...
for i,j in zip(human_files_short, dog_files_short):
    predict_humanfile.append(dog_detector(i))
    predict_dogfile.append(dog_detector(j))
    
print('human_files_short中图像检测到狗狗的百分比:{:.2f}%'.format(Counter(predict_humanfile)[True]/len(predict_humanfile)*100))
print('dog_files_short中图像检测到狗狗的百分比:{:.2f}%'.format(Counter(predict_dogfile)[True]/len(predict_dogfile)*100))
...
# __回答:__ 
# * 搭建步骤
#     * 添加卷积层，16个特征图，3* 3 的卷积核，卷积步长默认为1，填充默认为非边缘填充，以图像的维度作为输入维度
#     * 添加最大池化层，2* 2 的池化核，缩减特征维度
#     * 添加添加 BatchNormalization 层来降低Covariate Shift并加速运算过程
#     * 添加激活函数增强非线性表达能力
#     * 下两个模块与其上类似，分别使用32、64个特征图
#     * 添加全局平均池化层，相比Flatten，GlobalAveragePooling2D可以大量减少模型参数，降低过拟合的风险，同时显著降低计算成本
#     * 添加随机丢弃层，防止过拟合
#     * 添加全连接层，以 softmax 作为激活函数，将特征相应值归一化为概率
#     https://blog.csdn.net/wmsbeijing/article/details/53304362
#     
# * 原因说明：
#     * 首先，搭建三层卷积层可以检测更高级的特征，以达到狗狗品种分类的目的
#     * 同时，两个卷积层之间的池化层有效降低了数据的复杂度，使得训练效率得到有效提升
#     * 再者，通过一系列防止过拟合的手段增强模型的泛化能力
...
#   * 该模型使用预训练的 Xception 模型作为固定的图像特征提取器，其中 Xception 最后一层卷积层的输出被直接输入到架构的模型
# * Xception_model.add(GlobalAveragePooling2D(input_shape=train_Xception.shape[1:]))
#   * 使用全局平均池化层，减少参数个数
# * Xception_model.add(Dropout(0.2))
#   * 使用随机丢弃，防止模型过拟合
# * Xception_model.add(Dense(133, activation='softmax'))
#   * 使用全连接层，将输出的特征以概率的形式对应到狗的种类上
#   
#   
# 
# * 为什么相比普通的CNN，迁移学习可以取得更好的效果？
# 
#   * 相比普通的 CNN，迁移学习利用研究者精心搭建的卷积神经网络模型，在 ImageNet 中海量的图像进行训练，将图像的深层特征尽可能地提取出来，模型的架构，经过反复的调整，无论从计算效率和准确性方面均趋于完善，使用者只需依据实际需要更改全连接层加以训练，同时大大节省了搭建模型和训练模型的时间，因此迁移学习的性能要优于普通的 CNN
#   
#   
# * 为什么第三步中的尝试没有迁移学习的效果好？
# 
#   * 第三步中未使用迁移学习，而是自主搭建卷积神经网络，对于网络中的参数，都是依据经验进行的初步尝试，而图像中的特征究竟要分解到何种程度，是搭建者不知道的，因此参数的选择具有一定的主观性，不能很好地分解图像的特征，因此造成了识别准确性不高的结果
#   
#   
# * 为什么使用 Xception?
# 
#     * 比传统的卷积神经网络（例如 VGG）复杂性更低，因此参数在一定程度上得到减小，计算效率也更高
#     * 在增加网络深度的同时，不会出现梯度消失的问题，此特点与 Resnet50 相同
#     * Xception 是 Inception架构的扩展，它用深度可分离的卷积代替了标准的 Inception模块，在 ImageNet 数据集上，Xception 的表现稍稍优于 Inception v3，而且在大规模的图像分类数据集上的表现更好
...
### 然后返回此模型所预测的狗的品种
from extract_bottleneck_features import *
def Xception_predict_dogs(img_path):
    # 提取bottleneck特征
...
display(test_files[10])
img_path = test_files[10]
display(Xception_predict_dogs(img_path))
...
def distinguish_object(img_path):
    print('你的输入图片为:')
    display(Image(img_path, width=300, height=300))
    if dog_detector(img_path):
        print('检测到狗！图中的狗看起来像......')
        print(Xception_predict_dogs(img_path))
...
# 1. 输出结果比你预想的要好吗 :) ？或者更糟 :( ？
# 2. 提出至少三点改进你的模型的想法。
# * 输出的结果比我预想的要好，人和狗都能准确辨别，而输入是猫或兔子时，也能检测出非人或非狗
# * 改进既有模型的想法：
#  * 在训练模型时，通过数据增强，即图像的平移和旋转，增加模型的鲁棒性
#  * 选用其他网络结构或调整超参数，使模型达到更好的验证准确率
#  * 修改实现方式，能在1张图中识别多个物体，并分别进行预测
...
## 自由地使用所需的代码单元数吧
from IPython.core.display import Image
image_names = ['boy','girl','Chihuahua','Alaska','cat','rabbit']
for i in image_names:
    print('正在进行第{}次测试......'.format(image_names.index(i) + 1))
    print('输入的对象是一个{}'.format(i))
    img_path = './test_images/' + i + '.jpg'
    distinguish_object(img_path)
    print('**************************')
